...
class TcpSgtpTransport implements IProtocolTransport {
  ...
  Future<void> connect() async {
    ...
    _log.debug('Connecting to {host}:{port} (tls={useTls}, sni={sni})',
        parameters: {
          'host': host,
          'port': port,
          'useTls': useTls,
    ...
    try {
      if (useTls) {
        ...
        _log.debug('Starting TLS handshake with {host}:{port}',
        ...
        if (tlsServerName.isNotEmpty &&
          ...
          s = await SecureSocket.secure(
            ...
            onBadCertificate: (cert) {
              _log.error(
                  'TLS cert rejected by Dart: subject="{subject}" issuer="{issuer}" valid={validFrom}-{validTo}',
                  parameters: {
        ...
        _log.debug('TLS handshake OK: cert-subject={subject}',
            parameters: {'subject': secure.peerCertificate?.subject ?? 'none'});
      } else {
        s = await Socket.connect(host, port);
      }
    } catch (e) {
      _log.error('Connect failed to {host}:{port} (tls={useTls}) [{type}]: {error}',
          parameters: {
            'host': host,
            'port': port,
            'useTls': useTls,
    ...
    _log.debug('Socket established {host}:{port} (tls={useTls})',
        parameters: {'host': host, 'port': port, 'useTls': useTls});
        ...
        if (!headerDone.isCompleted) {
          ...
          if (headerBuf.length >= discoveryHeaderLen) {
            ...
            if (all.length > discoveryHeaderLen) {
              final bytes = Uint8List.fromList(all.sublist(discoveryHeaderLen));
              _inbound.add(bytes);
    ...
    try {
      ...
      _log.error('Banner wait failed on {host}:{port} (tls={useTls}): {error}',
          parameters: {'host': host, 'port': port, 'useTls': useTls, 'error': e});
      rethrow;
    }
    _log.debug('Banner received on {host}:{port} (tls={useTls})',
        parameters: {'host': host, 'port': port, 'useTls': useTls});
  }
