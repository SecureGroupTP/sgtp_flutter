...
For Firebase/FCM builds, `android/app/google-services.json` is generated by
FlutterFire and intentionally ignored by git. Local setup:

```bash
flutterfire configure --project=sgtp-3a2b6 --platforms=android
```

GitHub Actions restores that file from the repository secret
`ANDROID_GOOGLE_SERVICES_JSON_B64`. Create it from the local file with:
